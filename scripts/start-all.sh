...
# + 信号機検出）を一発起動する。5 つの launch を 1 プロセスから束ねて起動し、Ctrl+C で
# まとめて停止する。boot 時から常駐させたいときは scripts/kuas-mechlab3.service を参照。
...
echo "起動(1/5): drivetrain（モーター driver, /dev/ttyACM0）"
ros2 launch kuas_mechlab3 drivetrain_launch.py &
pids+=("$!")

echo "起動(2/5): cameras -> http://0.0.0.0:8080/ (front=${FRONT_DEVICE} rear=${REAR_DEVICE})"
ros2 launch kuas_mechlab3 cameras_launch.py \
  front_device:="${FRONT_DEVICE}" rear_device:="${REAR_DEVICE}" &
pids+=("$!")

echo "起動(3/5): teleop WebSocket -> ws://0.0.0.0:9001"
ros2 launch kuas_mechlab3 teleop_launch.py &
pids+=("$!")

echo "起動(4/5): 録画コントロール -> http://0.0.0.0:9002/record/status (route=${ROUTE} operator=${OPERATOR})"
ros2 launch kuas_mechlab3 record_launch.py route:="${ROUTE}" operator:="${OPERATOR}" &
pids+=("$!")

echo "起動(5/5): 信号機検出 -> traffic_light_topic (team=${TEAM_NUMBER}, 前カメラトピック購読)"
