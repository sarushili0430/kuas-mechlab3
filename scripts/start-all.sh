#!/usr/bin/env bash
# ロボット一式（モーター driver + 前後カメラ + テレオプ WebSocket）を一発起動する。
# 3 つの launch を 1 プロセスから束ねて起動し、Ctrl+C でまとめて停止する。
...
echo "起動(1/3): drivetrain（モーター driver, /dev/ttyACM0）"
ros2 launch kuas_mechlab3 drivetrain_launch.py &
pids+=("$!")

echo "起動(2/3): cameras -> http://0.0.0.0:8080/ (front=${FRONT_DEVICE} rear=${REAR_DEVICE})"
ros2 launch kuas_mechlab3 cameras_launch.py \
  front_device:="${FRONT_DEVICE}" rear_device:="${REAR_DEVICE}" &
pids+=("$!")

echo "起動(3/3): teleop WebSocket -> ws://0.0.0.0:9001"
