...
The drive and servo packets share the port but use distinct end-of-packet bytes
('d' vs 'a'), so the firmware parses them independently and the servo path leaves
the 4-wheel format untouched. Keeping these as string<->data functions lets the
standalone pytest job cover them; SerialLink owns the actual port I/O on top.
"""
...
def format_servo_us(us1: float, us2: float) -> str:
    ...
    stops. This is a separate packet from ``format_setpoints`` ('d' EOP); the
    firmware parses the two independently, so the 4-wheel drive format is
    unaffected.
    """
...
def angle_to_us(
    ...
    absolute [SERVO_MIN_US, SERVO_MAX_US] safety band. Keeping this calibration
    on the Pi lets the per-servo angle span be tuned without reflashing the
    firmware. A degenerate min_deg == max_deg maps everything to min_us.
    """
    if max_deg == min_deg:
