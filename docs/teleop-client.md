...
### アーム・LED 指令（ラッチ; ドライブとは独立）

同じ WS に別形の JSON を送るとアーム／LED も操作できる。3 種のパーサ（`vx/wz` / `servo` / `led`）は**相互排他**なので同じソケットに混在させて安全。

- **アーム**: `{"servo": [<肩deg>, <肘deg>]}` — 2 要素・0–180°。`mbed_driver` が µs（500–2500）へ換算してサーボへ送る。位置は**保持**（ドライブのように 0 へ減衰しない）。
- **LED**: `{"led": true|false}` — 車載 LED の on/off。同じく**保持**。

ドライブは毎周期ストリームだが、**アーム・LED は変化時に 1 回**送ればよい（ファームが最後の値を保持）。`teleop_server` は受信を `servo_cmd`（`std_msgs/Float32MultiArray`）/ `led_cmd`（`std_msgs/Bool`）として publish し、`mbed_driver` が購読してシリアルへ渡す。
